export const videoData = {
    ...
    // Helper functions
    getVideosByCategory(categorySlug) {
      const category = this.categories.find(cat => cat.category.slug === categorySlug);
      return category ? category.contents : [];
    },
  
    getAllVideos() {
      return this.categories.flatMap(cat => cat.contents);
    },
  
    getVideoById(slug) {
      return this.getAllVideos().find(video => video.slug === slug);
    },
  
    getRelatedVideos(currentVideoSlug, currentCategorySlug) {
      const videos = this.getVideosByCategory(currentCategorySlug);
      return videos.filter(video => video.slug !== currentVideoSlug);
    }
  };